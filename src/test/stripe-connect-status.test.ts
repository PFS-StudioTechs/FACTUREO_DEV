...
import { handle, type Deps } from "../../supabase/functions/stripe-connect-status/index";
...
function makeDeps(overrides: Partial<Deps> & { callerId: string | null; fetchImpl?: typeof fetch }): Deps {
  const company = { id: "company-1", user_id: OWNER_ID, stripe_account_id: "acct_existing", stripe_onboarding_done: false };
...
describe("stripe-connect-status — ownership", () => {
  it("refuse (403) le statut Stripe d'une entreprise qui n'appartient pas à l'appelant", async () => {
    const fetchSpy = vi.fn();
    const deps = makeDeps({ callerId: ATTACKER_ID, fetchImpl: fetchSpy });
    const req = new Request("https://x/stripe-connect-status", {
      method: "POST",
  ...
  it("laisse passer (200) le propriétaire légitime de l'entreprise", async () => {
    const fetchSpy = vi.fn(async () => new Response(JSON.stringify({ charges_enabled: true }), { status: 200 }));
