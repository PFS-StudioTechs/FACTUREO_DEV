...
| 2026-07-23 | Branche `fix/luca-invoice-redirect-and-delete-messages` (basée sur `main`). **Bug signalé** : clic sur "Créer cette facture" dans le chat Luca ne faisait rien. Cause : `Invoices.tsx:59-68` lit `location.state.lucaPrefill` dans un `useEffect` à deps `[]` (mount uniquement) — si l'utilisateur discutait avec Luca en étant déjà sur `/factures`, `navigate('/factures', {state})` ne remonte pas le composant donc l'effet ne se relance jamais, le clic n'a visiblement aucun effet. Fix : deps `[location.state]`. **Feature demandée** : pouvoir supprimer des messages de l'historique Luca (devient long avec le temps). La table `luca_messages` était append-only (select+insert only, pas de policy DELETE) — migration `20260723093214_luca_messages_delete_own.sql` ajoute `luca_msg_delete_own` (propriétaire uniquement). `useLucaConversation.ts` : nouvelle fonction `deleteMessage(id)` (suppression optimiste + DB) ; découverte au passage que les id de messages générés côté client (`crypto.randomUUID()`, pour l'affichage optimiste pendant le streaming) ne correspondent jamais aux vrais id insérés côté serveur par `call-luca` — corrigé en resynchronisant les vrais id depuis la base juste après l'envoi (`resyncMessages`), sinon `deleteMessage` sur un message tout juste envoyé n'aurait supprimé rien en base (0 ligne affectée) et le message serait réapparu au prochain rechargement. Bouton corbeille discret ajouté sur chaque message dans `LucaMessage.tsx` (visible utilisateur et assistant), câblé dans `LucaPanel.tsx`. `tsc`/`build`/Vitest (137 tests) verts. |
