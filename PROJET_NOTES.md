...
| 2026-07-22 | Branche `feat/ged` (basée sur `main`, poussée, non fusionnée). Coffre documentaire : table `documents` (index pur Storage, RLS, `UNIQUE(storage_bucket, storage_path)`), backfill des documents déjà existants (notes de frais, kbis), branchement des 3 chemins d'écriture existants (upload note de frais/kbis, génération Factur-X — qui ne persistait jusque-là jamais le PDF, bug corrigé au passage), helper `computeDateConservationMin` (durées indicatives non certifiées), page `/coffre` avec ouverture par URL signée uniquement. 17 tests. |
